...
1. `/home/claude/SeguridadGeneral/00-gobernanza/01-politica-cicd-devsecops.md` (fases, criterios de bloqueo, roles, excepciones).
2. `/home/claude/SeguridadGeneral/00-gobernanza/03-ambientes-modos-y-aprobaciones.md` (qué exige cada modo A/B/B0).
3. `/home/claude/SeguridadGeneral/02-pipelines/README.md` y el workflow del stack en `/home/claude/SeguridadGeneral/02-pipelines/workflows/` (nombres de jobs fijos, pines por SHA, `permissions`, `concurrency`).
4. `/home/claude/SeguridadGeneral/01-seguridad/01-gestion-de-secretos.md` (nombres exactos de secretos y variables).
...
- Aplicar el estándar a repositorios nuevos con `/home/claude/SeguridadGeneral/03-scripts/bootstrap-repo.sh` y adaptar el resultado al proyecto (nombres de scripts, rutas de componentes, monorepos).
